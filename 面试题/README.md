...
七.
栈(stack)和堆(heap)
栈存的是 基本类型的值 和 引用类型的地址 可以直接使用.
堆存的是引用类型指向的最终数据

八. http缓存
http缓存  主要是减少网络请求的体积和数量 css html js

强制缓存  
        cache-control max-age: 毫秒数    1. if 判断 毫秒数还没有超时 直接拿本地缓存  (不请求、network 假 200  会有 from cache 标识)
        no-cache 不缓存                  2. else  超过的 或者  no-cache 走协商缓存判断
else {
    协商缓存
        后续请求 服务端判断资源标识 是否一致
        返回 200 为最新资源
